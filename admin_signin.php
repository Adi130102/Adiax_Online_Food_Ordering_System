...
if (isset($_POST['onadminsignin'])) {
    $sql = "SELECT * FROM `admin_tbl` WHERE `username`='$_POST[username_email]' OR `email`='$_POST[username_email]'";
...
if (isset($_POST['adminsignup'])) {
    $sql = "SELECT * FROM `admin_tbl` WHERE `username`='$_POST[username]' or `email`='$_POST[email]'";
    $result = mysqli_query($conn, $sql);
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($result_fetch['username'] == $_POST['username']) {
                ...
                echo "
                <script>alert(' - E-mail already taken ! ');
                    window.location.href = 'index.php';
                </script>";
            }
        } else {
            ...
            $query = "INSERT INTO `admin_tbl`(`fullname`, `username`, `email`, `Address`, `password`,`datetime`) VALUES ('$_POST[fullname]','$_POST[username]','$_POST[email]','$_POST[Address]','$password', current_timestamp())";
            if (mysqli_query($conn, $query)) {
                // if data is inserted successfully
                echo "
                <script>alert('Registration Successfully');
                window.location.href = 'admin_tbl.php';
                </script>";
            } else {
